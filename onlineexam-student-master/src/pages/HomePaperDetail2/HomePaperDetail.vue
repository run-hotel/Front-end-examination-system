<template>
  ...
  >
    ...
    <div class="notices_run">
      ...
      >
        <ul class="item">
          <li>最新公告消息:{{ examCalendar[0].noticeContent }}</li>
        </ul>
      </vue-seamless-scroll>
    </div>
    ...
    <div class="paper_detail">
      <div class="paper_title">作业名称：{{ paperInfo.paperName }}</div>
      ...
      <div class="paper_create_time">
        ...
        {{ paperInfo.paperCreateTime | date_format }}
      </div>
    ...
    <div class="paper_start">
      <mt-button
        type="primary"
        :class="{ complete_flag: currentPaperStatus == 1 ? true : false }"
        @click.native="toStartPaper"
        :disabled="currentPaperStatus == 1 ? true : false"
      >
        {{ currentPaperStatus == 1 ? "已完成" : "开始作业" }}
      </mt-button>
    </div>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    // 点击开始作业按钮
    toStartPaper() {
      MessageBox.confirm("确定开始作业吗?").then(
        action => {
          Toast({
            message: "请开始作答",
            duration: 1500
          });
          this.$router.replace("/home/paper/detail2/start/" + this.paperId);
        },
        () => {
          //点击取消按钮操作
        }
