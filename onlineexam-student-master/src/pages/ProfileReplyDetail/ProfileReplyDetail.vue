<template>
  <div class="profile_reply_detail">
    ...
    <div class="reply_detail">
      <ul
        v-if="feedbackInfo.length"
        class="reply_detail_ul"
        v-for="(item, index) in feedbackInfo"
        :key="index"
      >
        <li class="feedback_time_li">
          <span class="feedback_time">{{
            item.feedbackCreateTime | date_format
          }}</span>
        </li>
          ...
          <viewer>
            <img
              :src="
                userInfo.stuImgSrc
                  ? userInfo.stuImgSrc
                  : require('../../common/imgs/profile.jpg')
              "
              alt="学生头像"
            />
          </viewer>
        ...
        <li class="feedback_time_li" v-if="item.ano">
          <span class="feedback_time">{{ item.replyTime | date_format }}</span>
        </li>
        <li class="message_li" v-if="item.ano">
          ...
          <span>
            <i
              class="iconfont iconxiazai41 i_read"
              v-if="item.feedbackStatus === '2' ? true : false"
              >已读</i
            >
            <i class="iconfont iconxiazai41 i_noread" v-else>未读</i>
            <br />
            您好，我是管理员{{ item.admName }}，工号是{{
              item.ano
            }}，您的反馈留言已经给您答复。<br />回答内容是：
            <p
              v-html="
                item.admAnswer.replace(/\#[\u4E00-\u9FA5]{1,3}\;/gi, emotion)
              "
            ></p>
          </span>
        </li>
