  ...
  <div class="wrong">
    ...
    <div class="wrong_list_container" v-if="wrongPapersList.length">
      <mt-loadmore :top-method="loadTop" ref="loadmore">
        <div
          class="wrong_list"
          v-for="(item, index) in wrongPapersList"
          :key="item.paperId"
        >
          <div class="wrong_list_item" :class="{ corner_new: index == 0 }">
            ...
            <div class="wrong_item">
              最终成绩：<span class="wrong_important">{{
                item.score || 0
              }}</span
              >分{{ item.score == null ? '（强制关闭考试页面退出考试）' : '' }}
            </div>
            <div class="wrong_item">
              试卷题数：总共<span class="wrong_important">{{
                item.totalNum
              }}</span
            ...
            <div class="wrong_item">
              单选题：总共<span class="wrong_important">{{
                item.singleNum
              }}</span
              >道，答错<span class="wrong_important">{{
                item.singleErrorNum
              }}</span
              >道，每题<span class="wrong_important">{{
                item.singleScore
              }}</span
              >分
            </div>
            <div class="wrong_item">
              多选题：总共<span class="wrong_important">{{
                item.multipleNum
              }}</span
              >道，答错<span class="wrong_important">{{
                item.multipleErrorNum
              }}</span
              >道，每题<span class="wrong_important">{{
                item.multipleScore
              }}</span
            ...
            <div class="wrong_item">
              ...
              >道，每题<span class="wrong_important">{{ item.judgeScore }}</span
              >分
            </div>
            <div class="wrong_item">
              填空题：总共<span class="wrong_important">{{ item.fillNum }}</span
              >道，答错<span class="wrong_important">{{
                item.fillErrorNum
              }}</span
              >道，每题<span class="wrong_important">{{ item.fillScore }}</span
              >分
            </div>
            <div class="wrong_btn">
              <div>
                <mt-button
                  size="small"
                  type="danger"
                  @click.native="toWrongDetail(item.paperId)"
              ...
              <div>
                <mt-button
                  size="small"
                  type="primary"
                  @click.native="toScoreDetail(item.paperId)"
                  :disabled="item.score == null"
                  >查看成绩报告</mt-button
                >
              </div>
