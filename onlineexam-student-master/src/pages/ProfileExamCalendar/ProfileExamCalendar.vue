<template>
  <section class="exam_calendar">
    ...
    <div class="exam_calendar_detail" v-if="examCalendar.length">
      <ul
        class="exam_calendar_detail_ul"
        v-for="(item, index) in examCalendar"
        :key="index"
      >
        <li class="exam_calendar_time_li">
          <span class="exam_calendar_time"
            >发布时间：{{ item.noticeCreateTime | date_format }}</span
          >
        </li>
        <li class="message_li">
          <viewer>
          ...
          <span>
            ...
            <br />
            您好，我是{{ item.teaName }}，教工号是{{
              item.tno
            }}，现发布相关考试公告。 <br />内容如下： <br />{{
              item.noticeContent
            }}
          </span>
        </li>
