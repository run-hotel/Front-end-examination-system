<template>
  <section class="paper">
    ...
    <mt-loadmore v-if="papersInfo.length" :top-method="loadTop" ref="loadmore">
      <div
        class="paper_list"
        v-for="(item, index) in papersInfo"
        :key="item.paperId"
      >
        <div
          class="paper_list_item"
          :class="{
            corner_top: index == 0,
            corner_new: index == 1,
          ...
          <div class="paper_participate">
            ...
            <div class="paper_participate_right">
              <mt-button
                size="small"
                type="primary"
                @click.native="toPaperDetail(item.paperId)"
                >查看详情</mt-button
              >
            </div>
          </div>
    ...
    async correctSearch() {
      ...
      if (result.statu == 0) {
        ...
        if (this.paperTypeValue === '' || this.paperTypeValue === '3') {
          this.papersInfo = papersInfo.filter(
            item =>
          ...
          this.papersInfo = papersInfo.filter(
            item =>
              item.paperType === parseInt(this.paperTypeValue) &&
              item.paperDifficulty >= this.minDifficulty &&
              item.paperDifficulty <= this.maxDifficulty &&
              item.participateNum >= this.minJoinNum &&
              item.participateNum <= this.maxJoinNum
