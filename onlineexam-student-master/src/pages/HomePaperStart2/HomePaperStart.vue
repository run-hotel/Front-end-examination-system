...
export default {
  ...
  created() {
    ...
    this.getPaperInfoByPaperId(() => {
      ...
      //调用计时器方法，在作业信息数据请求成功后获取考试时长开始计时
      this.countTime();
  ...
  methods: {
    ...
    //插入学生成绩表成绩信息，包含三个字段，考试开始时间、学号和作业id
    ...
    //用户手动点击提交作业按钮，弹出确认框
    clickSubmit() {
      MessageBox.confirm("确定要提交作业吗?").then(
        action => {
    ...
    //点击返回按钮
    toBack() {
      MessageBox.confirm("系统将自动提交作业，请确认是否离开考试?").then(
        action => {
          Toast({
            message: "可到我的页面查看成绩",
            duration: 1500
