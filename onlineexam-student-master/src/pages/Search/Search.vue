  ...
  <div class="search">
    ...
    <form class="search_form" @submit.prevent="">
      ...
      <input
        ...
        class="search_submit"
        value="搜索"
        :class="isSelect ? 'opacity' : ''"
        @touchstart="toggleSelect"
        @touchend="toggleSelect"
      />
    </form>
    ...
    <section class="list" v-show="!noSearchPapers">
      <ul class="list_container">
        <router-link
          :to="'/home/paper/detail/' + item.paperId"
          tag="li"
          v-for="item in searchPapers"
          :key="item.paperId"
          class="list_li"
        >
          ...
          <section class="item_right">
            <div class="item_right_text">
              <p>
                ...
                <span
                  >试卷类型：{{
                    item.paperType == 1 ? '随机组卷' : '固定组卷'
                  }}</span
                >
              </p>
              <p>
                <span
                  >考试时长：{{ Math.round(item.paperDuration / 60) }}分钟</span
                >
                <span>参加人数：{{ item.participateNum }}人</span>
              </p>
              <p></p>
              <p>
                <span>发布时间：{{ item.paperCreateTime | date_format }}</span>
