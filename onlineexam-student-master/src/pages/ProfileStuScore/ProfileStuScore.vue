    ...
    <mt-loadmore v-if="scoreList.length" :top-method="loadTop" ref="loadmore">
      ...
      >
        <div class="score_list_item" :class="{ corner_new: index == 0 }">
          ...
          <div class="score_total_score">试卷总分：{{ item.totalScore }}分</div>
          <div class="score_item">
            开始考试：{{ item.startTime | date_format }}
          </div>
          <div class="score_item">
            结束考试：{{ item.endTime | date_format }}
          </div>
          <div class="score_item score_important">
            最终成绩：{{ item.score || 0 }}分{{
              item.score == null ? '（强制关闭考试页面退出考试）' : ''
            }}
          </div>
          <div class="score_item score_important">
          ...
          <div class="score_participate">
            <div class="score_participate_left">
              <img src="../../common/imgs/person.png" alt="" /><span
                style="color: #FF0000"
                >{{ item.participateNum }}</span
              >人已参加
            </div>
