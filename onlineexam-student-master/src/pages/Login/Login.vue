  ...
  <section class="login-container">
    ...
    <HeaderTop title="在线考试系统学生端">
      ...
      <div class="header_message" slot="right">
        <viewer>
          <img src="../../common/imgs/college.jpg" />
        </viewer>
      </div>
    </HeaderTop>
    ...
    <div class="login-wrap" v-show="showLogin">
      <mt-field
        label="学号"
        placeholder="请输入12位数字学号"
        v-model="sno"
        @blur.native.capture="checkSno"
      />
      <mt-field
        label="密码"
        placeholder="请输入密码"
      ...
      />
      <mt-button type="primary" size="large" @click.native="checkLogin"
        >登录</mt-button
      >
    ...
    <div class="register-wrap" v-show="showRegister">
      ...
      <mt-field
        ...
        :state="newPswConfirmState"
        @blur.native.capture="checkNewPswConfirm"
      />
      <mt-field
        label="姓名"
        placeholder="请输入姓名"
      ...
      <mt-field
        ...
        v-model="newPhone"
        :state="newPhoneState"
        @blur.native.capture="checkNewPhone"
      />
      <mt-field
        label="安全码"
        placeholder="用于密码找回至少6位数字"
        type="password"
        v-model="newSecurityCode"
        :state="newSecurityCodeState"
        @blur.native.capture="checknewSecurityCode"
      />
      <mt-field
        label="确认安全码"
        placeholder="请再次输入安全码"
        type="password"
        v-model="newSecurityCodeConfirm"
        :state="newSecurityCodeConfirmState"
        @blur.native.capture="checknewSecurityCodeConfirm"
        @keyup.enter.native="stuRegister"
      />
      <mt-button type="primary" size="large" @click.native="stuRegister"
        >注册</mt-button
      >
    ...
    <div class="find-psw-wrap" v-show="showFindPsw">
      <mt-field
        ...
        v-model="findSno"
        :state="findSnoState"
        @blur.native.capture="checkfindSno"
      />
      <mt-field
        label="手机号"
        placeholder="请输入手机号"
        type="tel"
        v-model="findPhone"
        :state="findPhoneState"
        @blur.native.capture="checkfindPhone"
      >
        <button class="get-captcha" @click="getCaptcha">获取验证码</button>
      </mt-field>
      <mt-field
        label="安全码"
        placeholder="请输入安全码校验找回"
        type="password"
        v-model="findSecurityCode"
        :state="findSecurityCodeState"
        @blur.native.capture="checkfindSecurityCode"
      />
      <mt-field
        label="新密码"
        placeholder="请输入新密码"
        type="password"
        v-model="findNewPsw"
        :state="findNewPswState"
        @blur.native.capture="checkfindNewPsw"
      />
      <mt-field
        label="确认新密码"
        placeholder="请再次输入新密码"
        type="password"
        v-model="findNewPswConfirm"
        :state="findNewPswConfirmState"
        @blur.native.capture="checkfindNewPswConfirm"
        @keyup.enter.native="stuFindPsw"
      />
      <mt-button type="primary" size="large" @click="stuFindPsw"
        >找回密码</mt-button
      >
  ...
  methods: {
    ...
    checkNewPswConfirm() {
      if (this.newPswConfirm === '') {
        ...
        this.newPswConfirm.length >= 6 &&
        this.newPswConfirm === this.newPsw
      ) {
    ...
    checknewSecurityCodeConfirm() {
      if (this.newSecurityCodeConfirm === '') {
        ...
        this.newSecurityCodeConfirm.length >= 6 &&
        this.newSecurityCodeConfirm === this.newSecurityCode
      ) {
    ...
    checkfindNewPswConfirm() {
      if (this.findNewPswConfirm === '') {
        ...
        this.findNewPswConfirm.length >= 6 &&
        this.findNewPswConfirm === this.findNewPsw
      ) {
    ...
    async stuFindPsw() {
      ...
      if (result.statu == 0) {
        MessageBox.confirm('设置新密码成功，是否跳转到登录页面?').then(
          (action) => {
            //点击确定按钮操作
            this.toLogin()
          },
