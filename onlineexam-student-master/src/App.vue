   ...
   /!*     setTimeout(function() {
    ...
    if (sessionStorage.getItem('singleAnswers')) {
      this.$store.dispatch(
        ...
        JSON.parse(sessionStorage.getItem('singleAnswers'))
      )
    }
    if (sessionStorage.getItem('multipleAnswers')) {
      this.$store.dispatch(
        'refreshMultipleAnswers',
        JSON.parse(sessionStorage.getItem('multipleAnswers'))
      )
    }
    if (sessionStorage.getItem('judgeAnswers')) {
      this.$store.dispatch(
        'refreshJudgeAnswers',
        JSON.parse(sessionStorage.getItem('judgeAnswers'))
      )
    }
    if (sessionStorage.getItem('fillAnswers')) {
      this.$store.dispatch(
        'refreshFillAnswers',
        JSON.parse(sessionStorage.getItem('fillAnswers'))
      )
    }
    if (sessionStorage.getItem('firstCurrentTime')) {
      this.$store.dispatch(
        'refreshFirstCurrentTime',
        Number(sessionStorage.getItem('firstCurrentTime'))
      )
    }
  },
  watch: {
    $route(to, from) {
      ...
      if (
        (from.path === '/profile' && to.path === '/search') ||
        (from.path === '/profile' && to.path === '/wrong') ||
      ...
      if (typeof WebSocket === undefined) {
        ...
        if (JSON.parse(sessionStorage.getItem('userInfo'))) {
          ...
          this.socket = new WebSocket(
            process.env.BASE_WEBSOCKET +
              JSON.parse(sessionStorage.getItem('userInfo')).sno
          )
          // 监听socket连接
