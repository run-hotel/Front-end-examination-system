<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span v-if="scope.row.teaLastLoginTime">{{
            scope.row.teaLastLoginTime | date_format
          }}</span>
...
import {
  ...
  reqSearchTeachersList,
  reqInsertTeacherInfo
} from '@/api/teacher'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        tno: [
          { required: true, message: '教工号为必填项', trigger: 'blur' },
          { min: 6, max: 6, message: '教工号必须为6位数字', trigger: 'blur' }
        ],
        teaPsw: [
          {
            required: true,
  ...
  methods: {
    ...
    async handleFilter() {
      ...
      if (
        this.listQuery.teaSex === null ||
        this.listQuery.teaSex === undefined
      ) {
        teaSex = undefined
      }
      let result = await reqSearchTeachersList(
        this.listQuery.tno,
        this.listQuery.teaName,
        teaSex
      )
      if (result.statu === 0) {
        this.total = result.data.length
