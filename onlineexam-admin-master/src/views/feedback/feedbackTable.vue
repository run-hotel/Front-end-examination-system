  ...
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-badge
            :is-dot="scope.row.feedbackStatus === '0'"
      ...
      <el-table-column label="留言内容" align="center">
        <template slot-scope="scope">
          <span
            v-html="
              scope.row.feedbackContent.replace(
                /\#[\u4E00-\u9FA5]{1,3}\;/gi,
                emotion
              )
            "
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.feedbackCreateTime | date_format }}</span>
        </template>
      </el-table-column>
      <el-table-column label="留言人" align="center" width="120">
        <template slot-scope="scope">
          <div
            :class="{
              read_bg_img: scope.row.feedbackStatus === '2',
              no_read_bg_img: scope.row.feedbackStatus === '1'
            }"
          >
            <viewer>
              <img
      ...
      <el-table-column label="回复内容" align="center">
        <template slot-scope="scope">
          <span
            v-if="scope.row.admAnswer"
            v-html="
              scope.row.admAnswer.replace(/\#[\u4E00-\u9FA5]{1,3}\;/gi, emotion)
            "
          />
      ...
      >
        <template slot-scope="scope">
          <span v-if="scope.row.replyTime">{{
            scope.row.replyTime | date_format
          }}</span>
          <span v-else>待回复</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogFormVisible" title="留言回复">
      <el-form
        ...
        label-position="left"
        label-width="90px"
        style="width: 400px;height: 350px;margin-left:50px;"
      >
        <el-form-item label="留言内容" prop="feedbackContent">
          <span
            v-html="
              temp.feedbackContent.replace(
                /\#[\u4E00-\u9FA5]{1,3}\;/gi,
                emotion
              )
            "
        ...
        <el-form-item label="回复内容" prop="admAnswer">
          ...
          <i
            id="emotionBtn"
            :style="{ color: showEmotion ? '#65b1ff' : '' }"
...
export default {
  ...
  methods: {
    ...
    async handleFilter() {
      ...
      if (
        this.listQuery.feedbackStatus === null ||
        this.listQuery.feedbackStatus === undefined
      ) {
        feedbackStatus = undefined
      }
      let admAnswer = this.listQuery.admAnswer
      if (
        this.listQuery.admAnswer === '' ||
        this.listQuery.admAnswer === undefined
      ) {
        admAnswer = undefined
      }
      ...
      if (
        ...
        this.listQuery.admName === undefined
      ) {
        admName = undefined
      }
