  ...
  <div class="app-container">
        ...
        <template slot-scope="scope">
          <el-form label-position="left" inline class="demo-table-expand">
            ...
            <el-form-item label="题目答案：">
              <span>{{ scope.row.shortAnswer }}</span>
            </el-form-item>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="dialogStatus">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="120px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="题目答案" prop="shortAnswer">
          <el-input v-model="temp.shortAnswer" :rows="5" type="textarea" />
        </el-form-item>
        <el-form-item label="答案解析">
...
<script>
import { reqGetShortList, reqSearchShortList, reqDeleteShort, reqInsertShortInfo, reqUpdateShortInfo } from '@/api/bankManage'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        shortAnswer: [{ required: true, message: '题目答案为必填项', trigger: 'change' }],
  ...
  methods: {
    async getList() {
      ...
      const result = await reqGetShortList()
      if (result.statu === 0) {
        this.langOptions = result.data.langOptions
        this.total = result.data.shortList.length
        this.list = result.data.shortList.filter((item, index) => index < this.listQuery.limit * this.listQuery.page && index >= this.listQuery.limit * (this.listQuery.page - 1))
      }
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.handleUpdateShort()
        }
      })
    },
    async handleUpdateShort() {
      const result = await reqUpdateShortInfo(this.temp)
      if (result.statu === 0) {
    ...
    async handleFilter() {
      ...
      const result = await reqSearchShortList(this.listQuery.content, langId, composeFlag)
      if (result.statu === 0) {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.insertShortInfo()
        }
      })
    },
    async insertShortInfo() {
      const result = await reqInsertShortInfo(this.temp)
      if (result.statu === 0) {
        this.dialogFormVisible = false
        this.$notify({
          title: '成功',
    ...
    async handleDeleteQue(row) {
      ...
      const result = await reqDeleteShort(shortId)
      if (result.statu === 0) {
        this.$message({
          message: result.msg,
          type: 'success'
