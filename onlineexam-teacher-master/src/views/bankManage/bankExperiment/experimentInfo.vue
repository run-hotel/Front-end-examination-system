<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="实验内容" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.content }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="dialogStatus">
      ...
      >
        ...
        <el-form-item label="实验要求" prop="judgeAnswer">
          <el-input v-model="temp.judgeAnswer" :rows="5" type="textarea" />
        </el-form-item>
        <el-form-item label="注意事项">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          @click="dialogStatus === '添加实验' ? createData() : updateData()"
        >
          {{ dialogStatus === '添加实验' ? '确认添加' : '确认编辑' }}
        </el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        content: [
          { required: true, message: '实验内容为必填项', trigger: 'change' }
        ],
        judgeAnswer: [
          { required: true, message: '实验要求为必填项', trigger: 'change' }
        ],
        langId: [
  ...
  methods: {
    async getList() {
      ...
      if (result.statu === 0) {
        ...
        this.list = result.data.judgeList.filter(
          (item, index) =>
    ...
    async handleFilter() {
      ...
      const result = await reqSearchJudgeList(
    ...
    confirmDeleteQue(row) {
      this.$confirm('确定删除该实验吗?若实验已被组成试卷则无法删除', '提示', {
    ...
    beforeUpload(file) {
      ...
      if (
        ...
        this.$message.error(
          '请上传正确的文件类型,.doc,.docx,.xls,.ppt,.xlsx,.pptx,'
        )
        return false
      }
    },
