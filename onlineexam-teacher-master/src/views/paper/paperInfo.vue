<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.paperCreateTime | date_format }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="操作"
    ...
    <!--固定组卷弹出框-->
    <el-dialog
      :visible.sync="fixedDialogFormVisible"
      title="固定组卷"
      style="margin-bottom: 20px"
    >
      <el-form
...
import { reqGetClassByTeacherId } from '@/api/subject'
...
export default {
  ...
  data() {
    return {
      ...
      temp: {
        kind: 1,
        tno: this.$store.getters.userInfo.tno,
        startTime: '',
        paperName: '',
        paperDuration: '',
      ...
      fixRules: {
        ...
        singleScore: [
          { required: true, message: '单选题分数为必填项', trigger: 'change' }
        ],
        multipleScore: [
          { required: true, message: '多选题分数为必填项', trigger: 'change' }
        ],
        judgeScore: [
          { required: true, message: '判断题分数为必填项', trigger: 'change' }
        ],
        fillScore: [
          { required: true, message: '填空题分数为必填项', trigger: 'change' }
        ]
      },
      rules: {
        langId: [
        ...
        singleNum: [
          { required: true, message: '单选题数目为必填项', trigger: 'change' }
        ],
        multipleScore: [
          { required: true, message: '多选题分数为必填项', trigger: 'change' }
        ],
        multipleNum: [
          { required: true, message: '多选题数目为必填项', trigger: 'change' }
        ],
        judgeScore: [
          { required: true, message: '判断题分数为必填项', trigger: 'change' }
        ],
        judgeNum: [
          { required: true, message: '判断题数目为必填项', trigger: 'change' }
        ],
        fillScore: [
          { required: true, message: '填空题分数为必填项', trigger: 'change' }
        ],
        fillNum: [
          { required: true, message: '填空题数目为必填项', trigger: 'change' }
        ]
      },
  ...
  methods: {
    async getList() {
      ...
      let result = await reqGetPaperList4(userInfo.tno)
      if (result.statu === 0) {
    ...
    async seePaperDetail(row, column, event) {
      ...
      this.minSubPaperTitle = `单选题${row.singleNum}道（每道${
        row.singleScore
      }分），多选题${row.multipleNum}道（每道${row.multipleScore}分），判断题${
        row.judgeNum
      }道（每道${row.judgeScore}分），填空题${row.fillNum}道（每道${
        row.fillScore
      }分）。`
      this.paperData = [
        {
