<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.startTime | date_format }}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="endTime"
  ...
  methods: {
    async getList() {
      ...
      if (result.statu === 0) {
        ...
        this.list = result.data.scoresList.filter(
          (item, index) =>
    ...
    async handleFilter() {
      ...
      if (
        this.listQuery.paperId == null ||
        this.listQuery.paperId == undefined
      ) {
        paperId = 0
      }
